...
describe("EncryptedHabitMoodTracker", function () {
  ...
  it("should batch add multiple records successfully", async function () {
    ...
    const moodProofs = await Promise.all(
      moods.map((_, i) => fhevm.generateProof(encryptedMoods[i].handles[0], contractAddress))
    );
    const habitProofs = await Promise.all(
      habits.map((_, i) => fhevm.generateProof(encryptedHabits[i].handles[0], contractAddress))
    );

    await expect(contract.connect(signers.alice).batchAddDailyRecords(
      encryptedMoods.map(e => e.handles[0]),
      encryptedHabits.map(e => e.handles[0]),
      moodProofs,
      habitProofs
    )).to.emit(contract, "RecordAdded").withArgs(signers.alice.address, 0);

    expect(await contract.getDayCount(signers.alice.address)).to.eq(3);
  });

  it("should return correct timestamp range for user records", async function () {
    const mood1 = 3;
    const habitCompletion1 = 75;
    const mood2 = 4;
    const habitCompletion2 = 90;

    // Add first record
    const encryptedMood1 = await fhevm.createEncryptedInput(await contract.getAddress(), signers.alice.address);
    await encryptedMood1.add32(mood1);
    const encryptedHabit1 = await fhevm.createEncryptedInput(await contract.getAddress(), signers.alice.address);
    await encryptedHabit1.add32(habitCompletion1);

    await contract.connect(signers.alice).addDailyRecord(
      encryptedMood1,
      encryptedHabit1,
      encryptedMood1.getProof(),
      encryptedHabit1.getProof()
    );

    // Add second record
    const encryptedMood2 = await fhevm.createEncryptedInput(await contract.getAddress(), signers.alice.address);
    await encryptedMood2.add32(mood2);
    const encryptedHabit2 = await fhevm.createEncryptedInput(await contract.getAddress(), signers.alice.address);
    await encryptedHabit2.add32(habitCompletion2);

    await contract.connect(signers.alice).addDailyRecord(
      encryptedMood2,
      encryptedHabit2,
      encryptedMood2.getProof(),
      encryptedHabit2.getProof()
    );

    const [earliest, latest] = await contract.getRecordTimestampRange(signers.alice.address);
    expect(earliest).to.be.lt(latest);
    expect(earliest).to.be.gt(0);
    expect(latest).to.be.gt(0);
  });

  it("should calculate daily averages for user records", async function () {
    const mood1 = 3;
    const habitCompletion1 = 75;
    const mood2 = 4;
    const habitCompletion2 = 90;
    const mood3 = 2;
    const habitCompletion3 = 60;

    // Add three records
    for (let i = 0; i < 3; i++) {
      const mood = i === 0 ? mood1 : i === 1 ? mood2 : mood3;
      const habit = i === 0 ? habitCompletion1 : i === 1 ? habitCompletion2 : habitCompletion3;

      const encryptedMood = await fhevm.createEncryptedInput(await contract.getAddress(), signers.alice.address);
      await encryptedMood.add32(mood);
      const encryptedHabit = await fhevm.createEncryptedInput(await contract.getAddress(), signers.alice.address);
      await encryptedHabit.add32(habit);

      await contract.connect(signers.alice).addDailyRecord(
        encryptedMood,
        encryptedHabit,
        encryptedMood.getProof(),
        encryptedHabit.getProof()
      );
    }

    // Test daily averages for 7 days (should use all 3 records)
    const [encryptedAvgMood, encryptedAvgHabits] = await contract.connect(signers.alice).getDailyAverages(7);
